...
/** MCP / HTTP4s bridge codegen — emits a `<ServiceName>Mcp.scala` source
  ...
  * (default `false`). Mb1 scope: scaffold + `Output.Singular` only. Other
  * variants stub with `mcpError(-32601, "method not implemented in Mb1")`
  * pending Mb2.
  *
  * The bridge:
...
final class DomainServiceMcpRenderer(domain: Domain) {
  ...
  /** Pattern-match arm. Mb1: only `Singular` dispatches to `call`; other
    * variants return an `mcpError` stub. The static `wrap` flag is derived
    * from `OutputWrapPolicy.isWrapped` (D6 / §3.2).
    */
  private def renderMatchArm(rpc: DefMethod.RPCMethod): String = {
    ...
    rpc.signature.output match {
      ...
      case _ =>
        s"""            case Some(`toolName_$mName`) =>
           |              Ok(mcpError(-32601, "method not implemented in Mb1"))""".stripMargin
    }
  }
}
