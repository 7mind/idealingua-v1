...
class MetaAggregates(context: IDLParserContext) {
  ...
  def cstarting[T](keyword: => P[Unit], defparser: => P[T])(implicit v: P[?]): P[(RawNodeMeta, ParsedId, T)] = {
    withMeta(starting(keyword, defparser)).map {
      case (m, (i, t)) => (m, i, t)
    }
  }

  def cblock[T](keyword: => P[Unit], defparser: => P[T])(implicit v: P[?]): P[(RawNodeMeta, ParsedId, T)] = {
