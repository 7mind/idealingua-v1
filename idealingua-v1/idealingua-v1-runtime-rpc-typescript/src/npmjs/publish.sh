...
if [[ ! "$CI_BRANCH_TAG" =~ ^v.*$ ]]; then
  NPM_TAG_ARGS+=(--tag latest)
fi

( cd ${pkgPath} && npm publish --provenance --access public "${NPM_TAG_ARGS[@]}" || exit 1 )
( cd ${pkgPath}-es && npm publish --provenance --access public "${NPM_TAG_ARGS[@]}" || exit 1 )
