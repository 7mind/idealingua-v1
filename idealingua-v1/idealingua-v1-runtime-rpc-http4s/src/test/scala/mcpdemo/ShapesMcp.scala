...
/** Platform-neutral MCP pointer for `Shapes`, mirroring what
  * `DomainServiceMcpRenderer` now emits: a lightweight
  * [[McpServiceResource]] naming the committed `mcp/Shapes.mcp.json`
  * classpath resource. The full `McpServiceMeta` is obtained on the JVM via
  * `McpServiceLoader.load(ShapesMcp.resource)`.
  */
object ShapesMcp {
  val resource: McpServiceResource = McpServiceResource(
    serviceId    = "mcpdemo.Shapes",
    resourcePath = "mcp/Shapes.mcp.json",
    toolPrefix   = "mcpdemo.Shapes.",
  )
}
