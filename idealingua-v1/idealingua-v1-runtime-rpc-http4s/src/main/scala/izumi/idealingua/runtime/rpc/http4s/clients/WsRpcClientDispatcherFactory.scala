...
class WsRpcClientDispatcherFactory[F[+_, +_]: Async2: Temporal2: Primitives2: UnsafeRun2](
  ...
  protected def createListener[ServerContext](
    ...
    override def onClose(websocket: WebSocket, code: Int, reason: String): Unit = {
      websocket.sendCloseFrame().await()
      socketRef.set(None)
    }
    ...
    override def onTextFrame(payload: String, finalFragment: Boolean, rsv: Int): Unit = {
      UnsafeRun2[F].unsafeRun(handler.processRpcMessage(payload)).foreach {
        response =>
      ...
      override private[clients] def requestAndAwait(
        ...
        F.bracket {
          ...
          _ =>
            for {
              _ <- F.syncThrowable {
                nettyWebSocket.sendTextFrame(printer.print(packet.asJson)).await()
              }
