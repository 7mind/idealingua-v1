...
trait WsContextSessions[F[+_, +_], RequestCtx, WsCtx] {
  ...
  def getSession(ctx: WsCtx): F[Throwable, Option[WsClientSession[F, ?]]]
  final def contramap[C](updateCtx: C => F[Throwable, Option[RequestCtx]])(implicit M: Monad2[F]): WsContextSessions[F, C, WsCtx] = new WsContextSessions[F, C, WsCtx] {
    override def getSession(ctx: WsCtx): F[Throwable, Option[WsClientSession[F, ?]]] = self.getSession(ctx)
...
object WsContextSessions {
  ...
  def empty[F[+_, +_]: IO2, RequestCtx]: WsContextSessions[F, RequestCtx, Unit] = new WsContextSessions[F, RequestCtx, Unit] {
    ...
    override def getSession(ctx: Unit): F[Throwable, Option[WsClientSession[F, ?]]]                                                        = F.pure(None)
  }
  ...
  class WsContextSessionsImpl[F[+_, +_]: IO2, RequestCtx, WsCtx](
    ...
    override def getSession(ctx: WsCtx): F[Throwable, Option[WsClientSession[F, ?]]] = {
      F.sync(synchronized(Option(idToSession.get(ctx))))
        .flatMap(F.traverse(_)(wsSessionsStorage.getSession))
        .map(_.flatten)
    }
    ...
    @inline private final def updateCtx(
      ...
      synchronized {
        ...
        val updated  = requestContext.flatMap(wsIdExtractor.extract(_, previous))
