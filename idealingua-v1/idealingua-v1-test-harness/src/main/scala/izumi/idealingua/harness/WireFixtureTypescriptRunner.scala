...
/** PR-03.3a — TypeScript-leg subprocess orchestration. T2 implementation. */
object WireFixtureTypescriptRunner {
  ...
  @nowarn("cat=unused")
  def runAll(repoRoot: Path, fixturesRoot: Path, harnessTypescriptDir: Path): Int = {
    val fixtures = WireFixtures.load(fixturesRoot)
    if (fixtures.isEmpty) return 0

    TypescriptDriverBridge.runDriver(harnessTypescriptDir, fixtures) match {
      case Left(crashMsg) =>
        throw new WireFixtureVerificationFailure(s"TS driver crashed: $crashMsg")
          ...
          byKey.get((f.wireId, f.scenario)) match {
            ...
            case Some(r) =>
              ...
              if (java.util.Arrays.equals(expected, actual)) {
                ...
                // Distinguish authoring error (whitespace) from semantic divergence:
                // if the fixture canonical (noSpaces via JSON parse) equals actual UTF-8, it's whitespace only.
  ...
  private def formatReport(total: Int, failures: Seq[Failure]): String = {
    ...
    sb.append(s"TS wire fixture verification failed ($total fixtures checked, ${failures.size} failed).\n")
    ...
    for (kind <- orderedKinds if byKind.contains(kind)) {
      val fs = byKind(kind)
      val hint = kind match {
        case FailureKind.UnknownWireId       => " — add Dispatch.ts entry"
