  ...
  private def outputEphemeral(serviceId: ServiceId, baseName: String, suffix: String, out: Output): Seq[TypeDef] = {
    out match {
      ...
      case o: Output.Alternative =>
        val (success, successId) =
          toOutDef(serviceId, baseName, ts.tools.goodAltSuffix, o.success) // outputEphemeral(serviceId, baseName, ts.tools.goodAltSuffix, o.success)
        val (failure, failureId) =
          toOutDef(serviceId, baseName, ts.tools.badAltSuffix, o.failure) // outputEphemeral(serviceId, baseName, ts.tools.badAltSuffix, o.failure)
