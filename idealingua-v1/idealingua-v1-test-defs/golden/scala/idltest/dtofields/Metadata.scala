  ...
  implicit val decodeMetadata: Decoder[Metadata] = Decoder.instance(c => {
    ...
    for (fname <- maybeContent; value = c.downField(fname); result <- fname match {
      ...
      case _ =>
        ...
        val alts = List("idltest.dtofields.Point", "idltest.dtofields.Metadata.Struct", "idltest.dtofields.PointLike.Struct").mkString(",")
...
object Metadata extends MetadataCirce {
  ...
  implicit object Metadata_downcast_extend_Point extends izumi.idealingua.runtime.IRTExtend[Metadata, Point] {
    class Call(private val _value: Metadata) extends AnyVal {
      def using(w: Int, h: Int, x: Int, y: Int, ownfield: String, `export`: Boolean): Point = {
        assert(_value.asInstanceOf[_root_.scala.AnyRef] ne null)
        Point(id = _value.id, name = _value.name, w = w, h = h, x = x, y = y, ownfield = ownfield, `export` = `export`)
      }
    }
