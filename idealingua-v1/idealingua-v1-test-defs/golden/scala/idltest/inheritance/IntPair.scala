...
trait IntPairCirce {
  ...
  implicit val encodeIntPair: Encoder.AsObject[IntPair] = Encoder.AsObject.instance {
    case v: IntPair.Struct =>
      Map("idltest.inheritance.IntPair.Struct" -> v).asJsonObject
  }
  implicit val decodeIntPair: Decoder[IntPair] = Decoder.instance(c => {
    val maybeContent = c.keys.flatMap(_.headOption).toRight(DecodingFailure("No type name found in JSON, expected JSON of form { \"type_name\": { ...fields } }", c.history))
    for (fname <- maybeContent; value = c.downField(fname); result <- fname match {
      case "idltest.inheritance.IntPair.Struct" =>
        value.as[IntPair.Struct]
      case _ =>
        val cname = "idltest.inheritance.IntPair"
        val alts = List("idltest.inheritance.IntPair.Struct").mkString(",")
