...
final case class UserId(value: java.util.UUID) extends AnyVal with izumi.idealingua.runtime.model.IDLGeneratedType with izumi.idealingua.runtime.model.IDLIdentifier {
...
object UserId extends UserIdCirce {
  def parse(s: String): UserId = {
    ...
    if (!s.startsWith("UserId#")) {
      val name = "IdentifierId:{idltest.substraction}/#UserId"
    ...
    UserId(value = parsePart[java.util.UUID](parts(0), classOf[java.util.UUID]))
  }
}
