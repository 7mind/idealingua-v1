...
/** MCP / HTTP4s bridge for `TestAliasServ`. Generated by
  ...
  */
object TestAliasServMcpRoutes {

  /** tools/list envelope loaded from classpath resource at first access
    * (D8(b)). The translator emits `mcp/TestAliasServ.mcp.json` alongside
    * this source file when `emitMcpBridge = true`.
    */
  private lazy val toolsListJson: Json = {
    val stream = getClass.getClassLoader.getResourceAsStream("mcp/TestAliasServ.mcp.json")
    require(stream != null, "Missing classpath resource: mcp/TestAliasServ.mcp.json")
    try parser.parse(scala.io.Source.fromInputStream(stream, "UTF-8").mkString).toTry.get
    finally stream.close()
  }

  private val toolName_getMassCoupons = "izumi.test.domain02.TestAliasServ.getMassCoupons"
  private val methodId_getMassCoupons = IRTMethodId(IRTServiceId("TestAliasServ"), IRTMethodName("getMassCoupons"))
  private val toolName_ifaceMethod = "izumi.test.domain02.TestAliasServ.ifaceMethod"
  private val methodId_ifaceMethod = IRTMethodId(IRTServiceId("TestAliasServ"), IRTMethodName("ifaceMethod"))
  private val toolName_testADTIdReturn = "izumi.test.domain02.TestAliasServ.testADTIdReturn"
  private val methodId_testADTIdReturn = IRTMethodId(IRTServiceId("TestAliasServ"), IRTMethodName("testADTIdReturn"))
  private val toolName_testADTIdImportedReturn = "izumi.test.domain02.TestAliasServ.testADTIdImportedReturn"
  private val methodId_testADTIdImportedReturn = IRTMethodId(IRTServiceId("TestAliasServ"), IRTMethodName("testADTIdImportedReturn"))
    ...
    HttpRoutes.of[F[Throwable, _]] {
      ...
      case req @ POST -> Root / "mcp" / "tools" / "call" =>
        req.as[Json].flatMap { body =>
          ...
          nameOpt match {
            case Some(`toolName_getMassCoupons`) =>
              call(req, argsJson, methodId_getMassCoupons, wrap = true, extractCtx, mux, dsl)
            case Some(`toolName_ifaceMethod`) =>
              call(req, argsJson, methodId_ifaceMethod, wrap = true, extractCtx, mux, dsl)
            case Some(`toolName_testADTIdReturn`) =>
              Ok(mcpError(-32601, "method not implemented in Mb1"))
            case Some(`toolName_testADTIdImportedReturn`) =>
              Ok(mcpError(-32601, "method not implemented in Mb1"))
            case Some(other) if other.startsWith("izumi.test.domain02.TestAliasServ.") =>
