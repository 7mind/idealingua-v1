...
/** MCP / HTTP4s bridge for `NestedAdtsService`. Generated by
  ...
  */
object NestedAdtsServiceMcpRoutes {

  /** tools/list envelope loaded from classpath resource at first access
    * (D8(b)). The translator emits `mcp/NestedAdtsService.mcp.json` alongside
    * this source file when `emitMcpBridge = true`.
    */
  private lazy val toolsListJson: Json = {
    val stream = getClass.getClassLoader.getResourceAsStream("mcp/NestedAdtsService.mcp.json")
    require(stream != null, "Missing classpath resource: mcp/NestedAdtsService.mcp.json")
    try parser.parse(scala.io.Source.fromInputStream(stream, "UTF-8").mkString).toTry.get
    finally stream.close()
  }

  private val toolName_adtNested = "izumi.test.domain02.NestedAdtsService.adtNested"
  private val methodId_adtNested = IRTMethodId(IRTServiceId("NestedAdtsService"), IRTMethodName("adtNested"))
    ...
    HttpRoutes.of[F[Throwable, _]] {
      ...
      case req @ POST -> Root / "mcp" / "tools" / "call" =>
        req.as[Json].flatMap { body =>
          ...
          nameOpt match {
            case Some(`toolName_adtNested`) =>
              Ok(mcpError(-32601, "method not implemented in Mb1"))
            case Some(other) if other.startsWith("izumi.test.domain02.NestedAdtsService.") =>
