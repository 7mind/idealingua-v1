...
@dataclass
class TypeInfo:
    qname: str                           # C# qualified name with outer types
...
def lower_wire(namespace: str) -> str:
    """Convert a C# namespace to its lowercase dotted wire form. This is a
    best-effort guess used only for enums (where the generator doesn't emit
    RTTI constants)."""
...
def resolve_qname(name: str, ctx_outer: List[str], ctx_ns: str, table: Dict[str, TypeInfo]) -> Optional[str]:
    ...
    while True:
        ...
        if not chain:
            break
        chain.pop()
    # Already may be fully qualified — check direct
    return None
