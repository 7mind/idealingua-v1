...
- [ ] **Q9: Coverage of `idealingua-v1-test-defs/src/main/resources/defs/main-tests/source/izumi/`
      and `overlaytest/` corpora.** The meta-plan §"Findings,
      idealingua-v1-test-defs" mentions these as cross-package
      fixtures. Are they in PR-03 baseline scope, or only the 22
      `idltest/*.domain` files? Recommendation: include them — they
      exercise cross-domain `wireId` paths (threat model item 2).
...
- Scope: create `idealingua-v1-test-harness` module per §9. Add
  `build.sbt` entry mirroring `idealingua-v1-test-defs` at
  `build.sbt:929`. Add a single empty `GoldenSpec` test class.
  Add the `regenerateGoldens` SBT task that walks
  `idealingua-v1-test-defs/src/main/resources/defs/main-tests/
  source/idltest/*.domain` and invokes
  `TypespaceCompilerBaseFacade.compile` for each (lang, domain),
  writing into `idealingua-v1-test-defs/src/main/resources/golden/<lang>/<domain>/`.
  Wire the task into `aggregate` near `build.sbt:1739–1761`.
- Dependencies: §11 Q2 resolved.
- Success: `sbt regenerateGoldens` runs end-to-end without error and
  produces a non-empty `golden/` tree. `sbt
  idealingua-v1-test-harness/test` passes (the empty
  `GoldenSpec`).
- Tree: new module under
  `idealingua-v1/idealingua-v1-test-harness/`, edits to top-level
  `build.sbt`.
